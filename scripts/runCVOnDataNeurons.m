...
function runCVOnDataNeurons(firingRates)
	
	%Set parameters
	binSizeForEntropyCalc = 50;
	numberOfNeuronsToTry = [3 6 9 12 15 18];
	...
	for numberOfNeurons = numberOfNeuronsToTry
	...
	ylabel('Likelihood');
	legends = [];
	for n = numberOfNeuronsToTry
		legends = [legends; ['Neurons = ' num2str(n)]];
	end
	legend(legends,'location', 'southeast');
	print('plots/likelihoodPerIterationNeuron.eps','-depsc');

	% Plot final likelihoods for all iterations
	figure();
	plot(numberOfNeuronsToTry, generalLikelihoods(numberOfIterations,:));
	title('Final likelihood per neuron');
	xlabel('Number of neurons');
	ylabel('Likelihood');

	print('plots/likelihoodPerNeuron.eps','-depsc');
end
